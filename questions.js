...
const DISTRACTORS={economy:['GDP','景気動向指数','消費者物価指数'],exchange:['円高','購買力平価','実質為替レート'],stock:['株主','配当金','株価'],fund:['信託銀行','販売会社','分散投資'],ideco:['NISA','ETF','給付'],behavior:['損失回避','心理的会計','代表性ヒューリスティック']};

const FINAL_REVIEW_QUESTIONS=EXTRA_FACTS.flatMap(([category,source,facts])=>facts.flatMap(([statement,answer],index)=>[
  {id:`final-${category}-${index+1}-a`,category,type:'choice',question:'資料の説明として最も適切なものはどれか。',choices:[statement,'資料には説明されていない。','常に元本が保証される。','どの場合にも同じ結果になる。'],answer:0,explanation:statement,source},
  {id:`final-${category}-${index+1}-b`,category,type:'choice',question:`次の空欄に入る最も適切な語句を選びなさい。\n${statement.replace(answer,'（　　　）')}`,choices:[answer,...DISTRACTORS[category]],answer:0,explanation:statement,source},
  {id:`final-${category}-${index+1}-c`,category,type:'choice',question:`資料内容に基づき、空欄に当てはまる語句はどれか。\n${statement.replace(answer,'（　　　）')}`,choices:[answer,...DISTRACTORS[category].slice().reverse()],answer:0,explanation:statement,source},
  {id:`final-${category}-${index+1}-d`,category,type:'choice',question:`次の説明に最も関係する語句を選びなさい。\n${statement}`,choices:[answer,...DISTRACTORS[category]],answer:0,explanation:statement,source}
]));
